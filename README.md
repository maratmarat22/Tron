...
Message состоит из двух частей: Header и Payload — заголовок и полезная нагрузка сообщения соответственно.
Формат данных Header — Enum Header,
формат данных PAYLOAD — string[].
При отправлении сообщение сериализуется из Message в строку, при получении — десериализуется из строки в Message. 
...
Сообщения можно разделить на две группы:
...
При получении сообщения сервер подбирает необходимый обработчик и по необходимости выделяет ему состояние сессии, текущий Unicaster или Multicaster.
Обработчик возвращает ответ, который сервер отправляет обратно клиенту.
Дальнейшее поведение сервера зависит от типа запроса. Например, при запросе CreateLobby, после успешной обработки сообщения и отправки ответа клиенту сервер переключается в мультикаст-режим.
